...
Для работы нужно использовать окружение с gstreamer-1.28 (На оригинальном оранже):
...
Далее перейти в каталог с файлом CMakeLists.txt, вызвать следующие команды:
```
cmake .
cmake --build .
```

Сгенерируется бинарник `media-center`, который запускается следующей командой:
...
## 6. Описание API, которые использует rest сервер для управления камерами

Подробные примеры запросов к камерам см. в [camera_api_examples.md](./camera_api_examples.md)

Структуры и типы [structs.md](./structs.md)