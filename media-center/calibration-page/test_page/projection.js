...
const MIN_SCALE = 1.0;   // не даём уменьшить меньше базового — иначе появляется пустота
const MAX_SCALE = 8.0;

let _projCanvasBase = { w: 0, h: 0 };
...
function initProjWarpCanvas() {
    ...
    function resizeCanvas() {
        ...
        _projCanvasBase.w = canvasW;
        _projCanvasBase.h = canvasH;
...
function _resizeCanvasBitmap() {
    const canvas = document.getElementById('projWarpCanvas');
    if (!canvas || !_projCanvasBase.w) return;

    const dpr = window.devicePixelRatio || 1;
    const s   = projView?.scale ?? 1;

    // Bitmap = базовый_размер * scale * dpr.
    // CSS-размер остаётся базовым → CSS-scale слоя растянет канвас в s раз, и
    // битмап попадёт в физический экран 1:1 (с поправкой на dpr).
    canvas.width  = Math.round(_projCanvasBase.w * s * dpr);
    canvas.height = Math.round(_projCanvasBase.h * s * dpr);
...
function _projHitPoint(nx, ny) {
    ...
    const s = (typeof projView !== 'undefined' && projView.scale) ? projView.scale : 1;
    // Базовая hit-zone 14 px в экранных координатах. Под scale делим, чтобы
    // в нормализованных координатах она соответствовала тому же экранному размеру.
    const HIT_PX = 14 / s;
...
function _projPointerDown(e) {
    ...
    // При Ctrl или Shift — это режим zoom/pan, точки не трогаем.
...
function _projDraw() {
    ...
    // Внутренние пиксели битмапа на 1 экранный пиксель.
    // bitmap = base * s * dpr, экранно после CSS-scale = base * s.
    // То есть 1 экранный пиксель = dpr внутренних → размеры * dpr.
    const PX = dpr;
    ...
    if (pts.length > 1) {
        ...
        if (pts.length === _currentMaxPoints()) ctx.closePath();
        ctx.stroke();
        ctx.setLineDash([]);
    }
...
function _applyWarpTransform() {
    ...
    if (projView.scale !== _lastAppliedScale) {
        ...
        _resizeCanvasBitmap();   // пересоздать битмап под новый scale и перерисовать
    } else {
        _projDraw();
    }
}
...
function _clampPan() {
    // Не пускаем содержимое наружу viewport (proj-warp-wrapper).
    const wrapper = document.getElementById('projWarpWrapper');
    const layer   = document.getElementById('uiCanvasLayer');
    if (!wrapper || !layer) return;

    const wRect = wrapper.getBoundingClientRect();
    // Базовый размер слоя (без трансформации) — равен размеру wrapper по offset'у,
    // так как layer стилизован inset:0.
    ...
    // Если контент меньше viewport — центрируем; если больше — ограничиваем.
    if (scaledW <= wRect.width)  projView.ox = (wRect.width  - scaledW) / 2;
    else                          projView.ox = Math.min(0, Math.max(wRect.width  - scaledW, projView.ox));
    if (scaledH <= wRect.height) projView.oy = (wRect.height - scaledH) / 2;
    else                          projView.oy = Math.min(0, Math.max(wRect.height - scaledH, projView.oy));
}

function _onWarpWheel(e) {
    if (!e.ctrlKey) return;
    e.preventDefault();
    ...
    // Точка курсора в координатах wrapper.
    const mx = e.clientX - rect.left;
    const my = e.clientY - rect.top;

    const prevScale = projView.scale;
    const factor    = e.deltaY < 0 ? 1.15 : 1 / 1.15;
    const newScale  = Math.min(MAX_SCALE, Math.max(MIN_SCALE, prevScale * factor));
    if (newScale === prevScale) return;

    // Удерживаем точку под курсором в той же позиции после зума:
    // (mx - ox') / s' === (mx - ox) / s   →   ox' = mx - (mx - ox) * s'/s
    const ratio = newScale / prevScale;
    projView.ox = mx - (mx - projView.ox) * ratio;
    projView.oy = my - (my - projView.oy) * ratio;
    projView.scale = newScale;

    _clampPan();
    _applyWarpTransform();
}

// Pan начинается, только если клик прошёл сквозь канвас (пустая зона)
// либо если мы держим средний/правый клик. Чтобы не конфликтовать с расстановкой точек.
function _onWarpPanDown(e) {
    // Pan: средняя кнопка, либо Shift+левая, либо когда зум активен и клик мимо точки.
    // Самый простой и предсказуемый вариант: средняя кнопка или Shift+левая.
...
function _onWarpPanUp() {
    if (!projView.panning) return;
    projView.panning = false;
    document.getElementById('projWarpWrapper')?.classList.remove('panning');
}

// Вызвать один раз при инициализации страницы — например, из initProjWarpCanvas
function initWarpZoomPan() {
    const wrapper = document.getElementById('projWarpWrapper');
    if (!wrapper) return;

    wrapper.addEventListener('wheel', _onWarpWheel, { passive: false });
    wrapper.addEventListener('pointerdown', _onWarpPanDown);
    window.addEventListener('pointermove', _onWarpPanMove);
    window.addEventListener('pointerup',   _onWarpPanUp);

    // Реагируем на ресайз wrapper, чтобы перецентрировать.
    new ResizeObserver(() => {
        _clampPan();
        _applyWarpTransform();
    }).observe(wrapper);
...
// Опциональная функция: сбросить вид (если хочешь кнопку "Reset zoom")
function projResetView() {
    projView.scale = 1;
    projView.ox    = 0;
    projView.oy    = 0;
    _clampPan();
