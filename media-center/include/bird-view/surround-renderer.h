...
namespace birdview {
	...
	private:
		...
		// Ось приближения в ручном режиме, логарифмическая: плюс сужает поле
		// зрения, минус отодвигает орбиту от машины
		float m_zoom_t = 0.0f;
		// Сужение поля зрения; производная от m_zoom_t, не меньше 1
		float m_zoom = 1.0f;
