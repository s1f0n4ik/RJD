	...
	*/
	...
	public:
		...
		bool save(const std::filesystem::path& file_path) const {
			...
			utility::pretty_print(oss, m_json);
			file << oss.str();
			return true;
		};
		...
		boost::json::object get_sender_json_item(const std::string& key) {
			...
			try {
				...
				auto has_pattern = contains_pattern_fileds(obj);
				...
				auto has_calibrated = contains_calibration_fields(obj);
	...
	public:
		...
		static bool contains_pattern_fileds(const boost::json::object& obj) {
			return
		...
		static std::string make_item_key(std::string id, int width, int height) {
