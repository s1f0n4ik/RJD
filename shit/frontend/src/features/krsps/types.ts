...
export interface GwGpsFix {
  ...
  // не CAN (шина молчит, координат нет).
  age_ms?: number;
}
...
export interface GwConnection {
  ...
  retrying?: boolean;
  // Ход переподключения: фаза, номер попытки и время до следующей. Старый шлюз полей не отдаёт.
  phase?: 'connected' | 'waiting' | 'connecting' | 'off';
  attempt?: number;
  retry_in_ms?: number;
}
